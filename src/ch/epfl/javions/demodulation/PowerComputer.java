...
public final class PowerComputer {
    ...
    public int readBatch(int[] batch) throws IOException {
        ...
        for(int iPowerSample = 0; iPowerSample < batchSize; iPowerSample++){
            ...
            currSample[iSample] = this.batch[iSample];
            currSample[(iSample + 1)%8] = this.batch[iSample + 1];
            if(iPowerSample >= 4){ // The following formula is P(n) but we added 8 to each index used in the formula ( because (iSample - 6)%8 == (iSample + 2)%8 and it's easier to calculate)
